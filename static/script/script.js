...
//     navContainer.append(`<button class="navbar-toggler" type="button">
...
let productItems = document.querySelectorAll('.owl-item')
...
button.forEach((e) => {
    e.addEventListener('click', () => {
        if (added) {
            e.children[1].style.transform = "translate(-110%) skew(-40deg)";
            added = false;
        } else {
            e.children[1].style.transform = "translate(0px)";
            added = true;
        }
...
let productImages = document.querySelectorAll('.product-gallery img')
productImages.forEach((e) => {
    e.addEventListener('click', (event) => {
        let imageSrc = e.src
        // document.querySelector('.product-image img').src = imageSrc
        let newImage = $('#' + e.id)
        console.log(newImage.attr('src'))
        let currentImage = $('.product-image img')
        $('#show_large_image_modal').attr('href', imageSrc)
        currentImage.fadeOut(function () {
            newImage.attr('src', currentImage.attr('src')).fadeIn(500);
            currentImage.attr('src', imageSrc).fadeIn(500);
...
passwordInput.addEventListener('input', (e) => {
    ...
    errorMessage.innerText = 'پسورد باید بیشتر از 8 کارکتر باشد';
    errorMessage.id = 'error-message'
    if (e.target.value.length < 8) {
        e.target.classList.add('password-error');
        ...
        e.target.classList.remove('password-error')
        $('.register-form button').attr("disabled", false);
        let error = document.getElementById('error-message').remove()
    }
})
